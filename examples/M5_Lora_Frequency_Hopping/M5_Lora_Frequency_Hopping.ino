...
#define F_START             410 // start freq
#define F_STEP              0.1 // 25 kHz step
#define F_CH                1150 // channels to rx hop
#define BPS                 1200
...
float scores[] = { -999, -999, -999, 999, 999, 999};
float freqs[] = { 0, 0, 0, 0, 0, 0};
...
void setup() {
  ...
  for (int c = 0; c < F_CH; c++) {
    ...
    if (rssi > scores[0]) {
      ...
      scores[0] = rssi;
      freqs[0] = f;
    } else if (rssi > scores[1]) {
      scores[2] = scores[1];
      scores[1] = rssi;
      freqs[2] = freqs[1];
      freqs[1] = f;
    } else if (rssi > scores[2]) {
      scores[2] = rssi;
      freqs[2] = f;
    } else if (rssi < scores[3]) {
      scores[5] = scores[4];
      freqs[5] = freqs[4];
      scores[4] = scores[3];
      freqs[4] = freqs[3];
      scores[3] = rssi;
      freqs[3] = f;
    } else if (rssi < scores[4]) {
      scores[4] = scores[3];
      scores[3] = rssi;
      freqs[4] = freqs[3];
      freqs[3] = f;
    } else if (rssi < scores[5]) {
      scores[5] = rssi;
      freqs[5] = f;
    }
    Serial.print(F("Freq: "));
  ...
  M5.Lcd.drawString("1. " + String(freqs[0]) + " MHz, [rssi: " + String(scores[0]) + "]", 10, 10, 1);
  M5.Lcd.drawString("2. " + String(freqs[1]) + " MHz, [rssi: " + String(scores[1]) + "]", 10, 30, 1);
  M5.Lcd.drawString("3. " + String(freqs[2]) + " MHz, [rssi: " + String(scores[2]) + "]", 10, 50, 1);
  M5.Lcd.drawString("1. " + String(freqs[5]) + " MHz, [rssi: " + String(scores[5]) + "]", 10, 80, 1);
  M5.Lcd.drawString("2. " + String(freqs[4]) + " MHz, [rssi: " + String(scores[4]) + "]", 10, 100, 1);
  M5.Lcd.drawString("3. " + String(freqs[3]) + " MHz, [rssi: " + String(scores[3]) + "]", 10, 120, 1);
}
